  ...
  return (
    <Card className="flex flex-col h-full border-[#e6dfcb] hover:border-[#2c3324] hover:shadow-md transition-all group overflow-hidden">
      ...
      <CardHeader className="space-y-3 pb-3">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-1 min-w-0">
            {event.theme && (
              <span className="text-[11px] font-bold text-[#9a6423] uppercase tracking-wider block truncate">
                {event.theme}
              </span>
            )}
            <CardTitle className="text-lg group-hover:text-[#9a6423] transition-colors line-clamp-2">
              {event.title}
            </CardTitle>
          </div>
        </div>
      ...
      <CardContent className="space-y-3 flex-1 pb-4">
        <p className="text-xs sm:text-sm text-[#505748] line-clamp-2 leading-relaxed">
          {event.description}
        </p>

        <div className="space-y-1.5 pt-1 text-xs text-[#707666]">
      ...
      <CardFooter className="pt-2 border-t border-[#e6dfcb]/50 flex items-center justify-between text-xs font-semibold">
        <span className="text-[#8a9180]">
          {formatDate(event.startDate)}
        </span>
        <Link
          href={`/events/${event.slug}`}
          className="text-[#2c3324] group-hover:text-[#9a6423] flex items-center gap-1 transition-colors"
        >
