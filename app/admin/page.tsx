  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-serif text-3xl font-bold text-[#2c3324]">
            Administration Overview
          </h1>
          <p className="text-sm text-[#707666]">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Link href="/admin/ecclesias/new">
            <Button variant="outline" size="md" className="gap-2 shadow-xs bg-white">
              <Plus className="h-4 w-4" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <Card className="border-[#e6dfcb]">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <span className="text-xs font-semibold text-[#707666] uppercase tracking-wider">
              Ecclesias
            </span>
            <Church className="h-4 w-4 text-[#e0a861]" />
          </CardHeader>
          <CardContent>
            <div className="font-serif font-bold text-3xl text-[#2c3324]">
              {ecclesiasList.length}
            </div>
            <p className="text-[11px] text-[#707666] mt-1">Active fellowships in PH</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-[#e6dfcb]">
          ...
          <CardContent>
            ...
            <p className="text-[11px] text-[#707666] mt-1">Live camps in database</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-[#e6dfcb]">
          ...
          <CardContent>
            ...
            <p className="text-[11px] text-[#707666] mt-1">Active store catalog items</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-[#e6dfcb]">
          ...
          <CardContent>
            <div className="font-serif font-bold text-3xl text-[#9a6423]">
              {pendingOrders.length}
            </div>
            <p className="text-[11px] text-[#707666] mt-1">GCash receipts queue</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-[#e6dfcb]">
          ...
          <CardContent>
            ...
            <p className="text-[11px] text-[#707666] mt-1">Registered Brethren</p>
          </CardContent>
        </Card>
      </div>

      {/* Dynamic Site Metric Setting: Youth & Friends */}
      <Card className="border-[#e0a861]/50 bg-gradient-to-r from-[#fefcf1] to-[#f8f4e3]">
        <CardHeader>
          <div className="flex items-center gap-2 text-[#9a6423]">
          ...
          <form action={updateYouthCountAction} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 max-w-md">
            <div className="flex-1">
              <Input
                ...
                required
                className="bg-white font-mono font-bold text-base"
              />
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="border-[#e6dfcb]">
          <CardHeader className="flex flex-row items-center justify-between pb-4">
            ...
            <Link href="/admin/ecclesias" className="text-xs font-semibold text-[#9a6423] hover:underline">
              Manage All &rarr;
            </Link>
          </CardHeader>
          <CardContent>
            {ecclesiasList.length === 0 ? (
              <p className="text-xs text-[#707666]">No ecclesias registered yet.</p>
            ) : (
              <div className="space-y-3">
                {ecclesiasList.slice(0, 4).map((ecc) => (
                  <div
                    key={ecc.id}
                    className="p-3.5 rounded-xl bg-[#f8f4e3] border border-[#e6dfcb] flex items-center justify-between gap-3"
                  >
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-[#2c3324] truncate">{ecc.name}</p>
                      <p className="text-[11px] text-[#707666] truncate">{ecc.city}, {ecc.region}</p>
                    </div>
        ...
        <Card className="border-[#e6dfcb]">
          <CardHeader className="flex flex-row items-center justify-between pb-4">
            ...
            <Link href="/admin/events" className="text-xs font-semibold text-[#9a6423] hover:underline">
              View All &rarr;
            </Link>
          </CardHeader>
          <CardContent>
            {eventsList.length === 0 ? (
              <p className="text-xs text-[#707666]">No events created yet.</p>
            ) : (
              <div className="space-y-3">
                {eventsList.slice(0, 4).map((evt) => (
                  ...
                  >
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-[#2c3324] truncate">{evt.title}</p>
                      <p className="text-[11px] text-[#707666] truncate">{evt.location}</p>
                    </div>
