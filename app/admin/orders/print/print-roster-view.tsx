...
export function PrintRosterView({ orders }: PrintRosterViewProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-[#e6dfcb] shadow-xs">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <Link
                href="/admin/orders"
                className="text-xs text-[#707666] hover:text-[#2c3324] inline-flex items-center gap-1"
              >
      ...
      {/* Printable Sheet */}
      <div className="bg-white p-8 rounded-2xl border border-[#e6dfcb] shadow-sm print:shadow-none print:border-none print:p-0 space-y-6">
        {/* Printable Header */}
        <div className="border-b-2 border-[#2c3324] pb-4 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              <span className="font-serif font-bold text-xl text-[#2c3324]">
                PCYC PHILIPPINES &bull; MERCHANDISE ROSTER
              </span>
              <span className="text-xs font-semibold px-2 py-0.5 rounded bg-[#f8f4e3] border border-[#e6dfcb] text-[#2c3324]">
                {filterMode === 'EVENT_PREORDERS'
            ...
            <p className="text-xs text-[#505748] pt-1">
        ...
        {filteredOrders.length === 0 ? (
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b-2 border-[#2c3324] text-[#2c3324] uppercase tracking-wider font-bold">
            ...
            <tbody className="divide-y divide-[#e6dfcb]">
              {filteredOrders.map((ord, idx) => {
                ...
                return (
                  <tr key={ord.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-[#fefcf1]'}>
                    {/* Desk Claim Checkbox */}
                    <td className="py-3 px-2 text-center align-top">
                      <div className="h-5 w-5 border-2 border-[#2c3324] rounded mx-auto inline-block print:border-black" />
                    </td>

                    {/* Order Reference */}
                    <td className="py-3 px-2 font-mono font-bold text-[#2c3324] align-top whitespace-nowrap">
                      {ord.orderNumber}
                    </td>

                    {/* Member Name */}
                    <td className="py-3 px-2 align-top">
                      <strong className="text-[#2c3324] block">
                        {shipping?.recipientName ||
                          (userProfile ? `${userProfile.firstName} ${userProfile.lastName}` : 'PCYC Member')}
                      </strong>
                      <span className="text-[10px] text-[#707666]">
                        {shipping?.contactNumber || userProfile?.phoneNumber || 'N/A'}
                      </span>
                    </td>

                    {/* Ecclesia / Location */}
                    <td className="py-3 px-2 align-top text-[#505748]">
                      <span>{userProfile?.ecclesia || shipping?.city || 'Ecclesia'}</span>
                    </td>

                    {/* Merch Items & Selected Sizes */}
                    <td className="py-3 px-2 align-top">
                      <div className="font-semibold text-[#2c3324]">{itemsText || '1x Merch Item'}</div>
                      {shipping?.notes && (
                        <div className="text-[10px] text-[#707666] italic mt-0.5 line-clamp-1">
                          {shipping.notes}
                        </div>
                      )}
                    </td>
                    ...
                    {/* Total Amount */}
                    <td className="py-3 px-2 text-right align-top font-mono font-bold text-[#2c3324] whitespace-nowrap">
                      {formatCurrency(Number(ord.totalAmount))}
                    </td>

                    {/* GCash Verification */}
                    <td className="py-3 px-2 align-top font-mono text-[11px] text-[#505748] whitespace-nowrap">
        ...
        <div className="pt-6 border-t border-[#e6dfcb] flex justify-between text-[11px] text-[#707666]">
